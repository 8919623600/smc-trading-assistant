...
class SMCTradingEngine:
    def __init__(self, min_rr=2.0, max_rr=8.0, atr_multiplier=1.0, news_buffer_mins=15):
        self.min_rr = min_rr
        self.max_rr = max_rr
    ...
    def check_1h_liquidity_sweep(self, df_1h: pd.DataFrame, bias: str) -> dict:
        ...
        if bias == "BULLISH" and last_low:
            if current_bar['low'] < last_low and current_bar['close'] > last_low:
                return {"swept": True, "type": "BULLISH", "sweep_level": current_bar['low']}
        
        elif bias == "BEARISH" and last_high:
            if current_bar['high'] > last_high and current_bar['close'] < last_high:
                return {"swept": True, "type": "BEARISH", "sweep_level": current_bar['high']}
    ...
    def evaluate_1m_entry(self, df_1m: pd.DataFrame, sweep_info: dict, poi_info: dict) -> dict:
        ...
        if sweep_info["type"] == "BULLISH":
            stop_loss = sweep_info["sweep_level"] - (atr_val * self.atr_multiplier)
        ...
        return {"action": "WAIT", "reason": "Setup conditions met but Risk-to-Reward invalid"}
    ...
    def analyze(self, data_dict: dict, news_events: list = None) -> dict:
        ...
        data_dict requires keys: '4H', '1H', '15M', '1M' as Pandas DataFrames.
        """
        # Safe time extraction (Handles index vs time column)
        df_1m = data_dict['1M']
        ...
        if news_events:
            for event_time in news_events:
                if abs((current_time - event_time).total_seconds()) <= (self.news_buffer_mins * 60):
                    return {"decision": "NO_TRADE", "reason": f"News Blackout Active near {event_time}"}
        ...
        if not sweep["swept"]:
            return {"decision": "WAIT", "reason": f"No 1H Liquidity Sweep for {bias} bias"}

        # 3. 15M POI & Retest
        poi = self.detect_15m_poi(data_dict['15M'], sweep)
        if not poi["valid_poi"]:
            return {"decision": "WAIT", "reason": "15M POI not confirmed or retested"}

        # 4. 1M Precision Entry
